...
export const feedAliases = pgTable(
  ...
  (t) => [
    // 🔥 absolute uniqueness (critical)
    uniqueIndex("uniq_feed_alias_url").on(t.aliasUrl),

    // 🔥 lookup performance
    index("idx_feed_alias_source").on(t.sourceId),
  ],
);
...
export const sourcesRelations = relations(sources, ({ many }) => ({
  posts: many(posts),
  aliases: many(feedAliases),
}));

// Feed aliase relations
export const feedAliasesRelations = relations(feedAliases, ({ one }) => ({
  source: one(sources, {
    fields: [feedAliases.sourceId],
    references: [sources.id],
  }),
}));
